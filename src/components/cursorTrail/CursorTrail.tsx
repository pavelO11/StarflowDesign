...
const CursorTrail: React.FC = () => {
  ...
  const polyRef = useRef<SVGPolylineElement>(null);
  const gradientRef = useRef<SVGLinearGradientElement>(null);
  // Добавляем состояние для отслеживания видимости
  ...
  useEffect(() => {
    ...
    // Отслеживаем добавление/удаление класса hide-cursor на body
    const checkCursorVisibility = () => {
      setIsHidden(document.body.classList.contains('hide-cursor'));
    };

    // Запускаем MutationObserver для отслеживания изменений классов на body
